...
optim_gum = torch.optim.Adam(
        lr=args.lr, 
        params=[{'params':generator.feat_projector.parameters()},
                {'params':generator.gating.parameters()}],
        betas=(0.5, 0.999),
        amsgrad=True)
...
for e in range(args.epoch):
    ...
    for i in tqdm(range(loop_per_epoch)):
        ...
        dist_loss = criterion(dist, target)
        dist_loss.backward()
        optim_gum.step()
