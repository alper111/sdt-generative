...
for e in range(NUM_OF_EPOCHS):
    ...
    for i in tqdm(range(loop_per_epoch)):
        ...
        d_real, c_real = d_real[:, 0:1], d_real[:, 1:]
        d_real_loss = bce_with_logits(d_real, torch.ones_like(d_real, device=device))
        
        # train discriminator with fake data
        x_fake = generator(torch.randn(args.batch_size//8, z_dim, device=device))
        d_fake = discriminator(x_fake)
        d_fake, c_fake = d_fake[:, 0:1], d_fake[:, 1:]
        labels = torch.arange(8, device=device, dtype=torch.int64)
        labels = labels.repeat_interleave(args.batch_size//8)
